...
class Command(BaseCommand):
    help = _("Initialize the workdir to run the demo of betterbuysite.")
    ...
    def handle(self, verbosity, *args, **options):
        ...
        if os.path.isfile(initialize_file):
            self.stdout.write("Initializing project betterbuysite")
            call_command('makemigrations', 'betterbuysite')
            call_command('migrate')
            ...
            self.stdout.write("Project betterbuysite already initialized")
            call_command('migrate')

    def create_polymorphic_subcategories(self):
        from cms.models.pagemodel import Page
        from shop.management.commands.shop import Command as ShopCommand
        from betterbuysite.models import Commodity, SmartCard, SmartPhoneModel
